...
static u32int initrd_read(fs_node_t *node, u32int offset, u32int size, u8int *buffer)
{
    ...
    k_memcpy(buffer, (u8int*) (header.offset+offset), size);
    return size;
}
...
static struct dirent *initrd_readdir(fs_node_t *node, u32int index)
{
    ...
    if (node == initrd_dev && index == 0)
    {
      k_strcpy(dirent.name, "tty0");
      dirent.name[4] = 0;
      dirent.ino = 0;
      return &dirent;
    }
    ...
    dirent.name[k_strlen(root_nodes[index-1].name)] = 0;
